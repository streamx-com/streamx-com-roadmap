...
def load_user_map() -> dict[str, str]:
    """Jira accountId/email/displayName → GitHub login (from JIRA_GITHUB_USER_MAP secret)."""
    raw = (os.environ.get("JIRA_GITHUB_USER_MAP") or "").strip()
    if not raw:
        return {}
    data = json.loads(raw)
    if not isinstance(data, dict):
        raise ValueError("JIRA_GITHUB_USER_MAP must be a JSON object")
    return {str(k).strip(): str(v).strip() for k, v in data.items() if str(v).strip()}


def jira_assignee(epic: dict) -> dict | None:
    assignee = epic.get("fields", {}).get("assignee")
    return assignee if assignee else None


def github_login_for_assignee(assignee: dict | None, user_map: dict[str, str]) -> str | None:
    if not assignee or not user_map:
        return None
    for key in (assignee.get("accountId"), assignee.get("emailAddress"), assignee.get("displayName")):
        if not key:
            continue
        if key in user_map:
            return user_map[key]
        key_lower = key.lower()
        for map_key, login in user_map.items():
            if map_key.lower() == key_lower:
                return login
    return None


def sync_github_assignees(
    repo: str, issue_number: int, epic: dict, user_map: dict[str, str]
) -> None:
    if not user_map:
        return

    assignee = jira_assignee(epic)
    if not assignee:
        desired: list[str] = []
    else:
        login = github_login_for_assignee(assignee, user_map)
        if not login:
            desired = []
        else:
            desired = [login]
...
def jira_components(epic: dict) -> list[str]:
    return [
        c["name"].strip()
        for c in (epic.get("fields", {}).get("components") or [])
        if c.get("name")
    ]


def jira_fix_versions(epic: dict) -> list[str]:
    return [
        v["name"].strip()
        for v in (epic.get("fields", {}).get("fixVersions") or [])
        if v.get("name")
    ]


def github_label(value: str) -> str:
    """GitHub label name from Jira value (max 50 chars)."""
    label = value.strip()
    if len(label) > 50:
...
def metadata_labels(epic: dict) -> list[str]:
    """Jira component + fix version names as GitHub labels for filtering."""
    labels: list[str] = []
    for name in jira_components(epic):
        labels.append(github_label(name))
    for name in jira_fix_versions(epic):
        labels.append(github_label(name))
    return sorted(set(labels))


def jira_status_name(epic: dict) -> str:
    status = epic.get("fields", {}).get("status") or {}
    return (status.get("name") or "").strip()


def project_column_for_epic(cfg: dict, epic: dict) -> str:
    proj = cfg["github"]["project"]
    default = proj.get("default_column") or proj.get("column_value") or "Acknowledged"
    mapping = proj.get("status_to_column") or {}
    status = jira_status_name(epic)
    if not status:
        return default
    for jira_status, column in mapping.items():
        if status.lower() == str(jira_status).lower():
            return str(column)
    print(f"WARN: unmapped Jira status {status!r} → default column {default!r}", file=sys.stderr)
    return default
...
def find_github_issue(repo: str, jira_key: str) -> dict | None:
    issues = list_github_issues(repo, labels=[jira_key], state="all")
...
def create_or_update_issue(
    ...
    if existing:
        ...
        if (
            ...
            and set(all_labels) == existing_labels
        ):
...
def sync_epic(
    ...
    gh, action = create_or_update_issue(
        repo, key, str(title), str(body), labels + metadata_labels(epic)
    )
    col_value = project_column_for_epic(cfg, epic)
    meta = metadata_labels(epic)
    meta_s = f" | {', '.join(meta)}" if meta else ""

    item_id = ensure_project_item(pid, issue_node_id(repo, gh))
    set_board_column(pid, item_id, col_field, col_value)

    sync_github_assignees(repo, gh["number"], epic, user_map)

    write_jira_url(key, gh["html_url"], cfg)


def sync_single_epic(
    key: str, cfg: dict, repo: str, pid: str, col_field: str, labels: list[str], user_map: dict[str, str]
) -> None:
    key = key.strip().upper()
    ...
    sync_epic(published[0], cfg, repo, pid, col_field, labels, user_map)


def sync_all(
    cfg: dict, repo: str, pid: str, col_field: str, labels: list[str], user_map: dict[str, str]
) -> None:
    jql = epic_publish_jql(cfg, None)
    epics = jira_search_jql(jql, cfg)

    for epic in epics:
        sync_epic(epic, cfg, repo, pid, col_field, labels, user_map)
...
def main() -> int:
    ...
    if issue_key:
        sync_single_epic(issue_key, cfg, repo, pid, col_field, labels, user_map)
    else:
        sync_all(cfg, repo, pid, col_field, labels, user_map)
